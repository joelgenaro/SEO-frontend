...
const JobSearchOptions = () => {
  ...
  const getDatawithCurrentOption = async () => {
    ...
    axios
      .get(
        `http://localhost:8000/api/getData?page=${pageNumber}&country=${formData[0].value}&city=${formData[1].value}&town=${formData[2].value}&locality=${formData[3].value}&sectorOne=${formData[4].value}&sectorTwo=${formData[5].value}`
      )
      .then((res) => {
  ...
  const getDataWithText = () => {
    ...
    axios
      .get(
        `http://localhost:8000/api/getDataWithText?page=${pageNumber}&sector=${sectorSearch}&country=${countrySearch}`
      )
      .then((res) => {
        dispatch({ type: "UPDATE_DATA", payload: res.data.data });
        dispatch({ type: "UPDATE_LINKS", payload: res.data });
  ...
  function handleChange(type, value) {
    ...
    if (value != "") {
      ...
      axios
        .get(
          `http://localhost:8000/api/getSearchOptions?type=${type}&country=${formData[0].value}&city=${formData[1].value}&town=${formData[2].value}&locality=${formData[3].value}&sectorOne=${formData[4].value}`
        )
        .then((res) => {
          switch (type) {
            case "country":
